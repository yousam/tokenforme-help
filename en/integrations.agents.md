...
model    = "claude-opus-4-6"  # Use the actual supported model name
```
...
Using Visual Studio Code for macOS as an example：
```toml
...
- If the model name is rejected:
  ...
  - Use the correct model identifier shown in the documentation or console, such as `gpt-5.2` or `claude-opus-4-6`.
