...
联系客服咨询后选择最适合您的站点，各站点账号/余额/会员数据不相通。

## API BaseURL

在实际调用时，请根据所在区域选择对应的 API BaseURL，统一记作 **「API BaseURL」**：

- 国际站：`https://api.tokenfor.me`
- CN2区：`https://cn-api.tokenfor.me`
- UE1区：`https://ue1-api.tokenfor.me`

后续文档如无特殊说明，凡涉及具体调用域名的地方，统一表述为：**「参考『API 密钥』章节中的『API BaseURL』配置」**。请在各类工具中按照本小节的说明为所在区域配置对应地址。

## 联系方式

如需技术支持、售前、售后或充值相关咨询，可以通过以下方式联系我们：

- 客服QQ：`3928789892` 
- 客服Email：`service@tokenfor.me`
...
1. **注册并登录**：打开浏览器访问 `站点地址`，完成注册并登录控制台。
2. **创建 API Key**：在「API 密钥」中创建新的 Key，选择对应的 AI 厂商分组（倍率），模型列表后续可在「模型」板块查看（以控制台显示为准）。
3. **在工具中配置**：
