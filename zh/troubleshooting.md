...
本章节整理了在使用 「站点地址」 过程中常见的错误现象及排查步骤，建议在联系售后支持前先按本文逐项检查。
...
1. **API BaseURL是否正确？**
   - 参考「概览」章节中的「API BaseURL」小节，确认已在工具中使用正确的区域地址
2. **API Key 是否有效？**
   - 是否完整复制？有无多出空格或缺少字符？
...
1. 再次确认是否按照「API 密钥」章节中「API BaseURL」小节的说明选择了正确的 API BaseURL。
...
1. 在浏览器中尝试访问你所配置的 API BaseURL（参考「概览」章节中的「API BaseURL」小节），查看是否能正常打开（一般会返回一条简短信息或空白页）；
